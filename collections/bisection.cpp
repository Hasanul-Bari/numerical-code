...
{
    double a=x*x*x*x*x-5*x-1;    //write the equation whose roots are to be determined
    return a;
}
int main()
{
    cout.precision(8);        //set the precision
    cout.setf(ios::fixed);
    ...
    while (fabs(a-b)>=e)        /*if the mod of a-b is greater than the accuracy desired keep                         bisecting the interval*/
    {
        ...
        fb=f(b);
        fc=f(c);
        cout<<"a="<<a<<"     "<<"b="<<b<<"     "<<endl;/*print the                             values of a,b,c and fc  after each iteration*/
